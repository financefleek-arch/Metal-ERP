...
**Already on `main` (migration `0003`, commit `ac37b6a`) — this module wires into it, does not add it:**
- `party.status` (`active | archived`), `party.source` (`manual | inward_bill | tally_import`),
  `party.source_ref` (holds the `inward_bill.id` when `source=inward_bill`), `party.last_txn_at`
  (forward-only; invoice-finalize and **inward-approve** are its two writers).
- `PartySource` enum incl. `inward_bill`; `PartyStatus` enum; `ItemSource.auto_from_purchase`;
  `ItemStatus.unconfirmed` — all live in `app/models/_mixins.py`. **Do not re-add these.**
- `tenant.dormant_party_days INT DEFAULT 180` (dormancy window; not used by this module).
- `pg_trgm` GIN index on `party.legal_name` (name-fuzzy for supplier match — step 2 of X2 reuses it).

So the inward migration is **`0004`**, not `0003`.
...
| Screen | Phase | Status |
|---|---|---|
| Nav item "Inward" (visible only when `tenant.ext_inward_import`) | X0 | ✅ (`Shell.tsx`, gated on `me.ext_inward_import`) |
| **`InwardListPage`** (`/inward`) — upload drop-zone + bills table (status, confidence badges) | X4 | ✅ 1B rail + detail; polls while `extracting` |
| **`InwardReviewPage`** (`/inward/:id`) — PDF pane │ header form │ totals-check panel │ lines table with per-line match cell | X4 | ✅ `InwardReviewPane` (2B dense lines) |
| **Finalized review view** — read-only after Approve, **Download Tally XML** button, "save as template?" prompt | X5 / X6 | ✅ read-only + XML download (template prompt → X6) |
| **`InwardSettingsPage`** (`/inward/settings`) — the `tally_ledger_config` form | X6 | ✅ ledger names + encoding toggle |
| **`InwardDebugPage`** (`/inward/debug`) — dev-only PDF → XML tester | — | ✅ + backend `/api/inward-debug` HTML fallback |

**Build status (one big push, 2026-09-01):** X0–X5 + debug page landed. Fuzzy-only line
matching (LLM is a config-gated stub in `app/services/llm/`). Migration `0004`. The Sugal
Foods `INV2526-5667.pdf` is a committed fixture and extracts + reconciles to the paise in
`tests/inward/` (112 backend tests green; `mypy` + `ruff` clean; frontend builds).
Real-Tally-import validation (X5 blocking check-off) still pending — the XML is structurally
complete and balances; it needs one live import into Tally Prime. X6 template learn/apply and
X7 image path + batch are not built.
...
| Task | Detail |
|---|---|
| Enums | `app/models/_mixins.py`: `ExtractionMethod` (`einvoice_qr \| template \| table \| vision_llm`), `MatchMethod` (`exact \| alias \| fuzzy \| llm \| new \| manual`), `InwardStatus` (`uploaded \| extracting \| needs_review \| approved \| rejected \| error`), `SupplyType` (`intra \| inter`). Native-string style, matching the existing enums. `ItemSource.auto_from_purchase` and `PartySource.inward_bill` are **already there** — reuse, don't redefine. |
...
| Migration | `alembic revision --autogenerate -m "inward bill import: tables + tenant flag + job queue"` → hand-review → **`0004_*`** (`down_revision = "0003"`; `0003` is the party-provenance migration already on `main`). `alembic upgrade head`, `alembic check`, `alembic downgrade -1` all clean in CI (full chain against real PG16). |
...
three routes render empty shells, and every API endpoint exists (501). Migration `0004`
reversible down to `0003`.
...
| Task | Detail |
|---|---|
| `app/services/inward/resolve_supplier.py` | 1) **GSTIN exact** on `party.gstin` where `role in (supplier, both)` for the tenant → link (`matched_party_id`); a matched `customer` is staged for promotion to `both` on approve. 2) **No GSTIN on the PDF** → normalized-name trigram against supplier parties (reuses the `ix_party_legal_name_trgm` GIN index from `0003`); a single hit > 0.85 → *propose* (never auto-link). 3) **No match** → build `new_supplier_staged_json`: `legal_name` (header), `gstin`, `pan` (GSTIN chars 3–12 if it passes `PAN_RE`), `default_state_code` (prefix), `role = supplier`, `status = active`, one `party_address` (type `both`) from the header block. Not written until Approve. **Provenance is set at creation, not staged in the JSON** — see X5 step 2. |
...
| Tests | GSTIN hit → link; `customer` role → promotion staged (`role`, and `source`/`source_ref` **left untouched** — a promoted party keeps its original provenance); no GSTIN → trigram proposal, not linked; no match → staged party JSON has PAN + state derived correctly; inter-state detection from mismatched prefixes. |
...
| Task | Detail |
|---|---|
| `POST /api/inward-bills/{id}/approve` | One SQLAlchemy transaction: (1) re-validate reconciliation + every line resolved + supplier resolved (422 otherwise). (2) **Supplier provenance:** if a new party is staged → create it with `source = PartySource.inward_bill`, `source_ref = <this inward_bill.id>`, `role = supplier`, `status = active`, + its address. If a `customer` party was matched → promote `role` to `both` (leave `source` / `source_ref` as they were). **Either way**, set `matched_party.last_txn_at = bill_date` (forward-only — only if later than the current value; inward-approve is the second writer of this column alongside invoice-finalize). (3) Create the staged **new items** (`source = ItemSource.auto_from_purchase`, `status = ItemStatus.unconfirmed`, uom, `hsn_code`), normalized-key dedupe (a race that finds an existing normalized name links instead of creating). (4) Link every `inward_bill_line.matched_item_id`; bump `last_purchase_rate` / `last_purchased_at` (add these two columns to `item` in the `0004` migration — dormant until now). (5) Build the XML → write to the volume → set `tally_xml_path`. (6) `status = approved`. (7) `audit_log`. |
| `app/models` addition | `item.last_purchase_rate NUMERIC(15,2)`, `item.last_purchased_at TIMESTAMPTZ` — fold into the `0004` migration so X5 needs no new migration (or a small `0005` if X0 already shipped). |
...
| Tests | Approve creates exactly the staged masters and no more; a created supplier has `source = inward_bill` and `source_ref = <bill id>`; `party.last_txn_at` advances to `bill_date` but never moves backwards on an older bill; a promoted `customer→both` keeps its original `source`; re-approve is idempotent (409); XML snapshot test for the sample (golden file, normalized for the UDF id); intra vs inter produces CGST+SGST vs IGST; unreconciled bill → 422. |
...
| X0 Schema + flag + skeleton | 0.5 wk | migration `0004` (`0003` = party provenance, already shipped), router stubs, nav gating |
