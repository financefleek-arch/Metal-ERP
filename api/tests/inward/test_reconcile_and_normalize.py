...
class TestNormalize:
    ...
    def test_bartan_synonyms_hindi_canonical_spelling_only(self) -> None:
        from app.seed import SYNONYMS

        syn = dict(SYNONYMS)
        # spelling variants of one Hindi word converge on the canonical spelling
        assert normalize_name("SS Kadhai 10", syn) == "ss kadai 10"
        assert normalize_name("SS Karahi 10", syn) == normalize_name("SS Kadhai 10", syn)
        assert normalize_name("07 Fancy Mor Jhoola", syn) == "07 fancy mor jhula"
        assert normalize_name("Pital Lota", syn) == "pital lota"  # Hindi kept as-is
        # a Hindi noun is NOT turned into an English word
        assert "bucket" not in normalize_name("AL Balti 12", syn)
