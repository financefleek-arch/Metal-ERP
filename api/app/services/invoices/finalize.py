...
Finalize does NOT push to Tally. Pushing a finalized invoice to Tally is a
separate, explicit action (`POST /api/tally/invoices/{id}/push`, surfaced as
a "Push to Tally" button on the finalized invoice) so the shop chooses which
invoices cross into Tally.
"""
...
def finalize_invoice(
    ...
    for line, cl in zip(real_lines, computed.lines, strict=True):
        ...
        if it is not None:
            ...
            # "last billed wins": the item's default unit follows the most
    ...
    # No Tally push here — see module docstring. The shop pushes a finalized
    # invoice to Tally explicitly via POST /api/tally/invoices/{id}/push.
