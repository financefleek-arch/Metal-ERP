...
export function InwardReviewPane({ billId }: { billId: string }) {
  ...
  return (
    <div className="grid h-full grid-cols-[minmax(320px,420px)_1fr]">
      {/* PDF pane */}
      <div className="border-r border-line bg-[#d9d4ca] p-4">
        <embed
          src={`/api/inward-bills/${billId}/pdf#toolbar=1`}
          type="application/pdf"
          className="h-[calc(100%-1.5rem)] w-full rounded border border-[#b8b1a2] bg-white"
        />
        <p className="mt-2 text-center text-[11px] text-muted">{b.source_filename}</p>
      </div>
      ...
      <div className="flex flex-col gap-3 overflow-y-auto p-4">
        ...
        <div
          className={`rounded-xl border p-3 text-xs ${
            reconciled
              ? "border-ok/40 bg-[#e6efe8]"
              : "border-danger/40 bg-[#f4e3df]"
          }`}
        >
        ...
        {/* lines — 2B dense: all columns visible */}
